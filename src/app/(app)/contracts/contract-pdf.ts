...
function addText(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isLastLine && lines[i].trim().length > 0) {
      doc.text(lines[i], x, y, { align: 'justify', maxWidth });
    } else {
...
export async function generateContractPdf(
  ...
  y = addText(doc, introContratante, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.3;
  ...
  y = addText(doc, introContratada, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.8;
  ...
  y = addText(doc, objetoText, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.3;
  ...
  y = addText(doc, 'A CONTRATADA assume inteira responsabilidade pelos serviços técnicos que serão realizados, assim como pelas orientações técnicas que transmitir ao CONTRATANTE em função do objeto deste contrato.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;
  ...
  y = addText(doc, 'O CONTRATANTE compromete-se a observar e a cumprir rigorosamente todas as orientações técnicas transmitidas pela CONTRATADA, sob pena de eximir esta das consequências pela não observância e cumprimento.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;
  ...
  y = addText(doc, cl4, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.25;
  if (pagamento?.banco || pagamento?.agencia || pagamento?.conta || pagamento?.pix) {
    ...
    y = addText(doc, dadosBanc, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
    y += 0.25;
  }
  ...
  y = addText(doc, parag1, ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y = addText(doc, parag2, ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y = addText(doc, parag3, ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y = addText(doc, parag4, ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y += 0.4;

  y = addClauseTitle(doc, 'CLÁUSULA QUINTA - DO PRAZO', y, pageWidth, pageHeight, MB, drawWatermarkOnCurrentPage);
  y = addText(doc, 'Este instrumento é celebrado por tempo indeterminado, iniciando-se na assinatura do contrato e terminando na conclusão dos serviços relacionados na Cláusula Primeira.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;

  y = addClauseTitle(doc, 'CLÁUSULA SEXTA - DAS SANÇÕES E PENALIDADES', y, pageWidth, pageHeight, MB, drawWatermarkOnCurrentPage);
  y = addText(doc, 'No caso do não cumprimento da Cláusula Quarta, fica o CONTRATANTE sujeito à multa de 30% (trinta por cento) sobre o valor inadimplido. Persistindo a inadimplência, a CONTRATADA poderá retirar sua responsabilidade técnica sobre o processo oriundo deste contrato, eximindo-se de qualquer responsabilidade por eventuais danos decorrentes.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;

  y = addClauseTitle(doc, 'CLÁUSULA SÉTIMA', y, pageWidth, pageHeight, MB, drawWatermarkOnCurrentPage);
  y = addText(doc, 'Quando o processo for julgado e a licença for concedida, o cumprimento das condicionantes impostas pelos órgãos ambientais competentes será objeto de novo contrato, se necessário.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;

  y = addClauseTitle(doc, 'CLÁUSULA OITAVA', y, pageWidth, pageHeight, MB, drawWatermarkOnCurrentPage);
  y = addText(doc, 'Caso o órgão ambiental venha a solicitar informações complementares (durante o período de análise do projeto) que não sejam próprias do estudo elaborado, o valor para elaboração será combinado à parte. Para correção ou adequação de qualquer parte do estudo não será cobrado valor adicional do CONTRATANTE.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.4;
  ...
  y = addText(doc, cl9Body, ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y += 0.4;
  ...
  y = addText(doc, 'Para fins de complementação das obrigações contratuais, estabelecem as partes que: (I) O presente contrato vincula-se expressamente ao Processo, incluindo o cumprimento de PRADA e PTRF quando aplicáveis; (II) Os pagamentos sob responsabilidade do CONTRATANTE deverão ocorrer conforme a forma e datas acordadas na Cláusula Quarta; (III) Os valores constantes na proposta incluem os serviços técnicos prestados, abrangendo honorários, relatórios, monitoramentos, visitas técnicas e demais atividades necessárias à execução do objeto contratual; (IV) Todas as taxas, emolumentos e despesas vinculadas a órgãos ambientais serão de responsabilidade do CONTRATANTE.', ML, y, contentWidth, pageHeight, MB, 0.42, drawWatermarkOnCurrentPage);
  y += 0.4;

  y = addClauseTitle(doc, 'CLÁUSULA DÉCIMA SEGUNDA - DO FORO', y, pageWidth, pageHeight, MB, drawWatermarkOnCurrentPage);
  y = addText(doc, `Os casos omissos serão resolvidos de comum acordo. Para dirimir dúvidas ou conflitos, fica eleito o Foro da Comarca de ${foro.comarca || 'Unaí'} – ${foro.uf || 'MG'}.`, ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.6;

  y = addText(doc, 'E por estarem de comum acordo, assinam o presente instrumento em 02 (duas) vias de igual teor e forma.', ML, y, contentWidth, pageHeight, MB, 0.45, drawWatermarkOnCurrentPage);
  y += 0.8;
