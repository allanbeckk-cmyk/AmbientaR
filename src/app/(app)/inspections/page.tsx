...
export default function InspectionsListPage() {
  ...
  const handleDelete = async () => {
    ...
    const handleGeneratePdf = async (report: Inspection) => {
        ...
        const projectName = projectsMap.get(report.projectId)?.name || projectsMap.get(report.projectId) || 'N/A';
        pdfDoc.text(`Empreendimento: ${typeof projectName === 'string' ? projectName : 'N/A'}`, 15, yPos);
        yPos += 6;
        ...
        if(report.accompaniedBy || report.signatureUrl) {
            ...
            pdfDoc.setFontSize(12);
            pdfDoc.setFont('helvetica', 'bold');
            pdfDoc.text('Confirmação do Acompanhante', 15, yPos);
            yPos += 8;

            if(report.accompaniedBy) {
                pdfDoc.setFontSize(10);
                pdfDoc.setFont('helvetica', 'normal');
                pdfDoc.text(`Nome: ${report.accompaniedBy}`, 15, yPos);
                yPos += 8;
            }
            ...
            if(report.signatureUrl) {
                ...
                if(signatureBase64) {
                    try {
                        pdfDoc.addImage(signatureBase64, 'PNG', 15, yPos, 60, 30);
